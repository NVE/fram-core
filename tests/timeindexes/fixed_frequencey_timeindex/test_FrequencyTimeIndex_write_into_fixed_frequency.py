...
def test_when_target_vector_is_52_week_years_and_base_index_is_iso_time_should_return_vector_with_52_week_years_data():
    ...
    assert np.array_equal(target_vector, input_vector[:52]), "Target vector should contain the first 52 values of the input vector."
...
def test_when_target_vector_is_iso_time_and_base_index_is_52_week_years_should_return_vector_with_iso_time_data():
    ...
    assert np.array_equal(target_vector[:52], input_vector), "First 52 values of target vector should match input vector."
...
def test_when_input_time_index_coarser_resolution_then_target_time_index_should_disaggregate_to_target_index_resolution():
    base_index = FixedFrequencyTimeIndex(
        start_time=dt.datetime.fromisocalendar(2021, 1, 1),
        period_duration=dt.timedelta(weeks=1),
        num_periods=60,
        is_52_week_years=False,
        extrapolate_first_point=False,
    ...
    # The target vector should contain values repeated for each hour in the week from the input vector.
    expected_vector = np.repeat(input_vector, 7 * 12)
    assert np.array_equal(target_vector, expected_vector), "Target vector should contain repeated values for each hour in the week from the input vector."
...
def test_profile_time_index():
    ...
    assert np.array_equal(target_vector, np.array([1.0])), "Target vector should contain the mean value of the input vector for the specified year."

def test_when_time_index_is_one_year_and_iso_time_format_should_repeat_data_to_target_vector():
    base_index = FixedFrequencyTimeIndex(
        start_time=dt.datetime.fromisocalendar(2021, 1, 1),
        period_duration=dt.timedelta(weeks=1),
        num_periods=52,
    ...
    # The target vector should contain values repeated for each year from the input vector.
    expected_vector = np.tile(input_vector, 3)
    assert np.array_equal(target_vector, expected_vector), "Target vector should contain repeated values for each year from the input vector."

def test_when_time_index_is_one_year_and_52_week_years_should_repeat_data_to_target_vector():
    base_index = FixedFrequencyTimeIndex(
        start_time=dt.datetime.fromisocalendar(2020, 1, 1),
        period_duration=dt.timedelta(weeks=1),
        num_periods=52,
