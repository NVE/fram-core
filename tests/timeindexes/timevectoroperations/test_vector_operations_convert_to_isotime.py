...
from framcore.timeindexes._time_vector_operations import convert_to_isotime
...
@pytest.mark.parametrize(
    ("total_period", "period_duration", "expected_output"),
    [
...
@pytest.mark.parametrize(
    ("start_date", "total_period", "period_duration", "expected_output"),
    [
...
def test_period_does_not_contain_week_53_and_period_starts_not_at_start_of_year(
    start_date: datetime, total_period: timedelta, period_duration: timedelta, expected_output: np.ndarray,
):
    assert total_period % period_duration == timedelta(0)
...
def test_period_contains_whole_weeks_53_and_starts_not_at_start_of_year(
    start_date: datetime, total_period: timedelta, period_duration: timedelta, expected_output: np.ndarray,
):
    assert total_period % period_duration == timedelta(0)
...
@pytest.mark.parametrize(
    ("input_vector", "start_date", "period_duration"),
    [
...
def test_period_contains_whole_week_53_but_incompatible_data_resolution_raises_exception(
...
@pytest.mark.parametrize(
    ("period_duration", "expected_output"),
    [
